...
const gallery = () => {
  ...
  const handleDelete = async (imageName) => {
    console.log("Attempting to delete image:", imageName); // Log the image name for debugging

    try {
      const response = await fetch('http://localhost:5000/api/v1/gallery/delete', {
        method: 'POST', // POST method to send data in the body
        headers: {
          'Content-Type': 'application/json', // Ensure the body is in JSON format
        },
        body: JSON.stringify({ name: imageName }), // Pass the image name in the request body
      });

      if (!response.ok) {
        throw new Error('Failed to delete image'); // Throw error if the response is not successful
      }

      const data = await response.json(); // Parse the response data
      console.log(data.message); // Log the success message from the backend
      alert('Image deleted successfully!'); // Show a success message

      // Optionally refresh the image list after deletion
      fetchImages();

    } catch (error) {
      console.error('Error deleting image:', error); // Log any error that occurs during deletion
      alert(error.message || 'Failed to delete image. Please try again.'); // Show an error message
    }
  };
              ...
              <div key={image.id || index} className="relative w-full h-60 bg-gray-100 rounded-lg overflow-hidden shadow-md">
                ...
                <div className="absolute top-2 right-2 flex gap-2">
                  ...
                  <button
                      className="bg-white p-2 rounded-full shadow hover:bg-gray-100"
                      onClick={() => {
                        console.log("Deleting image:", image.name); // Log to check the value
