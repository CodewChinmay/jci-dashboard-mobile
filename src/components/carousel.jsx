...
const Carousel = () => {
  const [selectedImages, setSelectedImages] = useState([]); // Files selected for upload
  const [previewImages, setPreviewImages] = useState([]); // Local preview URLs for selected images
  ...
  const deleteImage = async (imageName) => {
    try {
      const response = await axios.delete(`https://jciamravati.in/api/v1/carousel/delete/${imageName}`);
  ...
  const handleImageChange = (e) => {
    ...
    // Validate: allow maximum 5 images
  ...
  // Remove a selected image from preview and selection
