...
const Workingdata = () => {
  ...
  // When editing, split the date into day, month, and year. Also split time if possible.
  const handleEdit = (data) => {
    ...
    // For time, check if the stored time includes AM/PM.
    let time = "";
    let timeMeridiem = "AM";
    if (data.time) {
  ...
  const handleUpdate = async (e) => {
    ...
    const { dateDay, dateMonth, dateYear, time, timeMeridiem } = editingData;
    ...
    // Combine time and meridiem into a single string (e.g., "12:30 PM")
    const formattedTime = `${time} ${timeMeridiem || "AM"}`;

    // Combine images array into a comma-separated string if needed
    ...
    // Remove temporary fields not needed in the payload
  ...
  return (
    <div
      className="p-6 bg-gray-50 overflow-hidden overflow-y-scroll scrollbar-custom"
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="bg-white p-4 sm:p-6 rounded">
          ...
          {/* Responsive Grid: 1 col on mobile, 2 on small, 3 on medium, 4 on large */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
            {filteredData.map((data) => {
              ...
              return (
                <div
                  key={data.id}
                  className="h-[400px] relative flex flex-col border border-gray-300 bg-white rounded-lg shadow-sm"
                >
                  {firstImage && (
                    <img
                  ...
                  {/* Actions div fixed at the bottom right */}
      ...
      {isModalOpen && editingData && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-4xl mx-4">
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Time with separate AM/PM select */}
                <div className="flex flex-col gap-2">
                  <label className="block text-sm font-medium text-gray-700">
                    Time
                  </label>
                  <div className="flex space-x-2">
                    ...
                    />
                    <select
                      value={editingData.timeMeridiem || "AM"}
                      onChange={(e) =>
                        setEditingData({ ...editingData, timeMeridiem: e.target.value })
                      }
